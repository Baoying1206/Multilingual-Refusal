...
for coeff in 0.5 1.0 2.0 5.0 10.0 20.0; do
    echo "=== Fixed Addition coeff=$coeff ==="
    python3 -c "
...
random.seed(42)
test_data = load_dataset_split(harmtype='harmful', split='test', lang='zh')
...
evaluation = evaluate_jailbreak(completions=completions, methodologies=['substring_matching'], evaluation_path=f'output/fixed_addition/zh/eval_coeff_{coeff}.json')
print(f'coeff={coeff}, ASR={evaluation[\"substring_matching_success_rate\"]:.4f}')
"
done
echo "Done!"
